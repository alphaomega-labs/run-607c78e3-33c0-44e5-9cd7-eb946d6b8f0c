...
Concretely, we study ASP-grounded neurosymbolic pipelines in which exact inference quality depends jointly on semantic correctness and solver efficiency \citep{src_011,src_012,src_014,src_016,src_019,src_033,src_039,src_045}. Prior work has established the structural determinants of complexity, especially treewidth, cycle structure, and compilation tradeoffs \citep{src_001,src_002,src_003,src_006,src_008,src_045,src_046,src_047,src_048}. What remains underdeveloped is an auditable pipeline that combines semantics-preserving rewrite acceptance with instance-level backend routing for exact execution.

AutoTW-ASP addresses that gap with an exact-first optimization layer. We do not claim that one backend or one rewrite family is universally superior. Instead, we formalize rewrite acceptance and backend selection as explicit decisions with measurable assumptions, theorem-backed guarantees where available, and empirical audits where guarantees are not available. This framing makes the method falsifiable and keeps the contribution closer to a reproducible systems procedure than to a benchmark-specific heuristic.

\textbf{Contributions.}
\begin{itemize}
\item We define a guarded rewrite objective for grounded ASP and prove a compositional semantic invariance theorem for accepted rewrite sequences.
...
\item We implement and evaluate the pipeline on six benchmark families with nineteen seeds, reporting semantic audits, runtime behavior, calibration outcomes, and negative results alongside positive ones.
\item We report theorem audits, uncertainty intervals, and reproducibility details, including where empirical thresholds remain unmet and how those gaps affect the conclusions.
\end{itemize}
...
We consider grounded normal ASP programs $\Pi$ with observation constraints $O$, following stable-model semantics commonly used in NeurASP-style formulations \citep{src_012,src_019,src_039}. Let $\mathrm{SM}(\Pi,O)$ denote the stable-model set under observation $O$, and let $y(\Pi,O)$ be a deterministic label extraction map from this set. Exactness requires preserving both the stable-model set and downstream labels for accepted transformations.

The optimization problem is built from four mathematical objects. First, program transformation is represented by a finite rewrite sequence $r$. Second, admissible sequences are constrained by guard predicates and collected in a feasible set $\mathcal{F}(\Pi,O)$. Third, execution after rewriting is delegated to an exact backend selector over $\mathcal{B}=\{\text{enum},\text{comp}\}$. Fourth, regime-level comparison uses an explicit utility score rather than an informal aggregate narrative. We introduce these objects in that order so later references do not depend on notation that has not yet been defined.

The method operates over a finite rewrite library $\mathcal{R}$ containing local transformations inspired by prior structure-aware reductions \citep{src_045,src_048,src_039}. For a rewrite sequence $r=(R_1,\ldots,R_m)$, let $\Pi^r=R_m(\cdots R_1(\Pi)\cdots)$ denote the transformed program and let $\Pi^{r_{<j}}$ denote the intermediate program after applying the first $j-1$ rules. For each candidate rewrite $R\in\mathcal{R}$ and program state, a guard predicate $\mathrm{Guard}(R,\Pi,O)\in\{0,1\}$ decides whether the rewrite is admissible. We define the feasible rewrite set
\begin{equation}
\label{eq:feasible}
\mathcal{F}(\Pi,O)=\left\{r=(R_1,\ldots,R_m): m\in\{0,\ldots,m_{\max}\},\; \mathrm{Guard}(R_j,\Pi^{r_{<j}},O)=1\;\forall j\in\{1,\ldots,m\}\right\},
\end{equation}
so the empty sequence is always available as a no-rewrite baseline.
...
This decomposition separates what is directly optimized from what is guaranteed. Rewrite optimization minimizes surrogate objective \eqref{eq:objective}, not unknown true runtime. Semantic invariance is guaranteed only for guard-admissible rules satisfying local equivalence assumptions. Routing is optimized using predicted costs in \eqref{eq:predcost} and decision rule \eqref{eq:router}, while regret guarantees depend on bounded prediction error as formalized later. This distinction matters for interpreting evidence: theorem statements certify conditional properties, whereas empirical results evaluate how often those conditions hold in executed workloads.

To make the feasible-set definition operational, we distinguish three guard classes. Syntactic guards reject transformations that violate rule-shape constraints for supported templates. Semantic guards enforce local equivalence obligations derived from prior cycle-aware and reduction analyses. Audit guards require bounded empirical checks on representative grounded subsets before global acceptance. This three-tier design combines familiar ingredients into one conservative admissibility policy that prioritizes exactness even when it sacrifices potential speed gains.
...
The formulation assumes finite grounded programs, decidable guards, deterministic label extraction from stable models, and comparable runtime traces across exact backends. It does not assume complete coverage of every arithmetic rewrite class, globally calibrated uncertainty under all ablations, or global dominance of one backend or one rewrite family. Those unresolved items are tracked as explicit gaps and influence claim support status. This scope mirrors prior structural and compilation analyses while making the manuscript-specific objectives explicit \citep{src_002,src_003,src_039,src_045,src_048}.
...
The formal component establishes three conditional guarantees that anchor the empirical narrative: compositional semantic invariance for accepted rewrite sequences under local guard-certified equivalence, existence of a rewrite minimizer over the guarded feasible set, and a two-action routing regret bound proportional to the largest prediction error. Together, these results specify what can be guaranteed independent of benchmark outcomes.
...
\label{lem:argmin}
Under the feasible-set definition in \eqref{eq:feasible}, the minimizer of \eqref{eq:objective} over $\mathcal{F}(\Pi,O)$ exists.
\end{lemma}
\begin{proof}
The rewrite library $\mathcal{R}$ is finite, the sequence length is bounded by $m_{\max}$, and the empty sequence belongs to $\mathcal{F}(\Pi,O)$ by \eqref{eq:feasible}. Hence $\mathcal{F}(\Pi,O)$ is finite and non-empty. Because $J(r)$ is real-valued for each feasible $r$, the set $\{J(r): r\in\mathcal{F}(\Pi,O)\}$ is finite and has a smallest element. Any feasible sequence attaining that value is a minimizer.
\end{proof}
...
We evaluate six benchmark families: MNIST sum-$k$, Sudoku $4\times4$, formula evaluation, ordering, counting, and shortest-path variants. Two families are materialized from a public neurosymbolic repository lineage, and four are generated with deterministic seeds and manifest-level checksums. This design balances realism and controlled structural sweeps.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{figures/main/regret_and_calibration_panels.pdf}
\caption{Routing regret and calibration diagnostics across routing policies. The left panel compares mean regret by feature set and fallback strategy. The right panel tracks the best observed envelope-violation rate and its associated tail runtime at each uncertainty threshold, showing that even the strongest setting remains above the target violation level.}
...
This work shows that exact neurosymbolic ASP optimization can be automated in a way that remains scientifically auditable. The key ingredients are explicit constrained rewrite optimization, backend routing, and separate diagnostics for correctness, efficiency, and reliability. Formal analysis establishes conditional invariance and regret envelopes; empirical evaluation shows where those guarantees translate into practical gains and where calibration or audit sensitivity still falls short.

The main claim is methodological rather than universal-performance-oriented. By expressing transformation and routing as explicit decision objects with measurable assumptions, we can improve exact inference behavior while preserving traceable semantics and clear caveats. The current results support that framing: guarded rewrites preserve semantics and improve runtime in the tested regimes, while routing calibration and cache auditing remain incomplete and should be treated as open engineering problems rather than settled strengths.
